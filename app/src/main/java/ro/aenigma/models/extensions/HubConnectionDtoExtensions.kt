...
object HubConnectionDtoExtensions {
    ...
    suspend fun HubConnectionDto.cleanup(): CleanupResult? {
        return connection?.invoke(CleanupResult::class.java, CLEANUP_METHOD)?.await()
    }

    @JvmStatic
    suspend fun HubConnectionDto.pull(): PullResult? {
        return connection?.invoke(PullResult::class.java, PULL_METHOD)?.await()
    }
    ...
    suspend fun HubConnectionDto.generateNonce(): GenerateTokenResult? {
        return connection?.invoke(GenerateTokenResult::class.java, GENERATE_NONCE_METHOD)?.await()
    }

    @JvmStatic
    suspend fun HubConnectionDto.start() {
        return connection?.start()?.await() ?: Unit
    }
